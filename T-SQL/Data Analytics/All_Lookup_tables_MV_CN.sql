...
WITH AllPurchases AS (
    ...
    isnull(flic2025.R3_Status,'Lapsed') as  R3_Status 
	...
	left join [dbo].[alllic_trout_2025] flic2025 on flic2025.Customerid=c.CustomerId
...
WITH TroutCustomers AS (
    SELECT
        *,
        ...
        ROW_NUMBER() OVER(PARTITION BY EmailAddress ORDER BY OrderDate DESC) as rn2
    FROM
        ##AllFishCustomers
    WHERE
